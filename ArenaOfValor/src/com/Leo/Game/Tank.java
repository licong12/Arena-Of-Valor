...
public class Tank extends Hero {
	 ...
	 * @param y Tank ordinate
	 * @param good Our side or the enemy
	 * @param robot Robot or user
	 */
	public Tank(int x, int y, boolean good, boolean robot) {
	...
	* @param x Mage abscissa
	 * @param y Tank ordinate
	 * @param good Ousr side or the enemy
	 * @param robot Robot or user
	 * @param dir Mage direction
	 * @param battleField battleField
	 */
	public Tank(int x, int y, boolean good, boolean robot, Direction dir, BattleField battleField) {
	...
	public void draw(Graphics g) {
		...
		case D:
			g.drawImage(YaSeImags[0], x, y, null);
			break;

		case U:
			g.drawImage(YaSeImags[1], x, y, null);
			break;
		case L:
			g.drawImage(YaSeImags[2], x, y, null);
			break;

		case R:
			g.drawImage(YaSeImags[3], x, y, null);
			break;
	...
	void move() {
		...
		if (y < 40)      
			y = 40;
		if (x + Tank.width > BattleField.Fram_width)  
			x = BattleField.Fram_width - Tank.width;
		if (y + Tank.length > BattleField.Fram_length)
			y = BattleField.Fram_length - Tank.length;
	 ...
	 * @return m
	 */
	public Swards TankFire(){  
		if (!live)
			return null;
		int x = this.x + Tank.width / 2 - Swards.width / 2; 
		int y = this.y + Tank.length / 2 - Swards.length / 2;
		Swards m = new Swards(x, y , good, Kdirection, this.battleField); 
		battleField.swards.add(m); 
